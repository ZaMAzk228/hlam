...
app.get('/allPosts', (req,res)=>{
    db.all("SELECT posts.id,users.name,posts.text,posts.user_id FROM posts JOIN users ON users.id=posts.user_id",(err, data)=>{
        res.json(data)
    })
})
...
app.post('/addPost', (req,res)=>{
    ...
    db.run(`INSERT INTO posts (user_id,text) VALUES(${id},'${text}')`)
    db.all("SELECT posts.id,users.name,posts.text,posts.user_id FROM posts JOIN users ON users.id=posts.user_id",(err, data)=>{
        res.json(data)
    })
})
